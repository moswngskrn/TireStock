...
<body>
      ...
      <div class="container">
        ...
        <div id="navbar" class="navbar-collapse collapse">
          <ul class="nav navbar-nav">
            ...
            <li  class="active"><a href="add_item.php">เพิ่มจำนวนสินค้าและราคา</a></li>
	...
	<div class="container box">
		...
		<rightside>
			<font size="6"><span class="label label-success">สินค้าที่ถูกเลือก</span></font>
			...
			<div id="showCart">
				<table class='table table-bordered table-striped'>
					...
					<tr>
						<th>รหัสสินค้า</th>
						<th width="5%">จำนวน</th>
						<th width="20%">ราคาสินค้ารวม</th>
						<th width="20%">ราคาสินค้าต่อชิ้น</th>
						<th width="5%">ลบ</th>
					</tr>
					</thead>
			...
			<form method="post" id="item_form" enctype="multipart/form-data">
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<input type="number" name="item_price_sum" id="item_price_sum" class="form-control" />
...
<script type="text/javascript" language="javascript" >
	...
	$(document).ready(function(){
		...
		var dataTable = $('#item_data').DataTable({
			...
			"ajax":{
				...
				type:"POST",
				data:{status:"เพิ่มจำนวน / ราคา", btn:"btn btn-sm btn-info"},
			},
			"columnDefs":[
				{
	...
	function add_helper() {
		...
		if (quantity == 0 || price == 0 || price_pp == 0) {
			modal_alert('คำเตือน !!', "กรุณากรอกข้อมูลให้ครบถ้วน");
			//alert("กรุณากรอกข้อมูลให้ครบ");
			return;
		} else if (parseInt(quantity) < 0 || parseInt(price) < 0 || parseInt(price_pp) < 0) {
	...
	$(document).on('click', '.update', function(){
		...
		for (var i = 0; i < cart.length; i++) {
			if (cart[i][0] == itm_id) {
				modal_alert('คำเตือน !!', "คุณได้ทำการเลือกสินค้าชนิดนี้เพื่อทำการเพิ่มจำนวนและราคาไปแล้ว ถ้าต้องการแก้ไขกรุณากดลบที่สินค้าชิ้นนี้และดำเนินการใหม่อีกครั้ง");
				//alert('คุณได้ทำการเลือกสินค้าชนิดนี้เพื่อทำการเพิ่มจำนวนและราคาไปแล้ว ถ้าต้องการแก้ไขกรุณากดลบที่สินค้าชิ้นนี้และดำเนินการใหม่อีกครั้ง');
				return;
			}
		}
		...
		if (cart.length == 0) modal_alert('คำเตือน !!', "คุณยังไม่ได้เลือกสินค้า");
			...
			$.ajax({
				...
				success:function(msg)
				{
					swal({
						title: "แจ้งเตือน",
  						text: "ระบบดำเนินการเพิ่มจำนวนสินค้าและราคาเรียบร้อยแล้ว",
					},function(isOk){
						if(isOk){
